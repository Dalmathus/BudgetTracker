...
IF EXISTS (
   SELECT 1
    ...
    WHERE type IN ('FN', 'IF', 'TF')
      AND name = 'dbf_create_code_from_desc'
)
   DROP FUNCTION dbo.dbf_create_code_from_desc
GO

SET QUOTED_IDENTIFIER OFF
GO

CREATE FUNCTION dbo.dbf_create_code_from_desc ( @pv_string_desc NVARCHAR(100))
RETURNS NVARCHAR(10)
AS
BEGIN

   IF LEN(@pv_string_desc) <= 10 BEGIN
