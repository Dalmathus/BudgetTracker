...
IF EXISTS (
   SELECT 1
     FROM sys.procedures
    WHERE name = 'dbp_create_new_transaction_category'
)
   DROP PROCEDURE dbo.dbp_create_new_transaction_category
GO

SET QUOTED_IDENTIFIER OFF
GO

CREATE PROCEDURE dbo.dbp_create_new_transaction_category @pn_account_id    NUMERIC(18),
...
BEGIN
   ...
   IF NULLIF(LTRIM(@v_category_code), '') IS NOT NULL
   OR NULLIF(LTRIM(@pv_category_desc), '') IS NOT NULL
   BEGIN
